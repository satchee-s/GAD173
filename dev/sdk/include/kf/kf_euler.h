...
namespace kf
{
	...
	public:
		...
		Euler(const kf::Vector2 &v) :angle(atan2(v.y, v.x)) {}
		...
		inline operator kf::Vector2()
		{
			return kf::Vector2(cos(angle), sin(angle));
		}

		// Convert a Vector2 into a euler angle.
		inline Euler &fromVector(const kf::Vector2 &v)
		{
			angle = atan2(v.y, v.x);
			return *this;
		}
